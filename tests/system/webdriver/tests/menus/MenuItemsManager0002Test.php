...
class MenuItemsManager0002Test extends JoomlaWebdriverTestCase
{
	...
	public function addMenu_OptionFunctionalityCheck_OptionWorking()
	{
		...
		$this->assertTrue($this->siteHomePage->itemExist($title, 'a'));
		$this->siteHomePage->itemClick($title);
		$this->assertTrue($this->siteHomePage->itemExist($article, 'h2//a'), 'Article Must be present');
		...
		$this->assertTrue($this->siteHomePage->checkVote());
		...
		$this->siteHomePage->itemClick($title);
		$this->assertFalse($this->siteHome3Page->itemExist("Published:", 'dl/dd[1]'));
		...
		$this->assertFalse($this->siteHomePage->checkVote());
